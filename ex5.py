...
def linear_classification(portion=1.):
    ...
    x_train_tfidf, x_test_tfidf = tf.fit_transform(x_train), tf.transform(x_test)  # vectorize the training documents

    logreg = LogisticRegression()
    logreg.fit(x_train_tfidf, y_train)
    y_pred = logreg.predict(x_test_tfidf)
    # pred_words = [category_dict[cat] for cat in y_pred]
    # for se nt, pred in
...
def transformer_classification(portion=1.):
    ...
    enc = tokenizer(x_train, truncation=True, padding=True)
    trainer = Trainer(
        model=model,
        args=TrainingArguments(num_train_epochs=5, learning_rate=5e-5, per_device_train_batch_size=16,
                               per_device_eval_batch_size=16, output_dir='./results'),
        tokenizer=tokenizer,
        compute_metrics=compute_metrics,
        train_dataset=Dataset(tokenizer(x_train, truncation=True, padding=True), y_train),
        eval_dataset=Dataset(tokenizer(x_test, truncation=True, padding=True), y_test)
    )
    trainer.train()
    trainer.evaluate()
